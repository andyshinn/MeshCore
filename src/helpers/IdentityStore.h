...
#elif defined(NRF52_PLATFORM) || defined(NRF54_PLATFORM) || defined(STM32_PLATFORM)
  // nRF54L: caveman99/nRF54_Arduino ships the same Adafruit_LittleFS and
  // InternalFileSytem libraries as the nRF52 core, so the abstraction is
  // unchanged -- only the underlying storage differs (RRAM, not flash).
